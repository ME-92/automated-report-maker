...
class Reader():
    ...
    def read_twitter_data(self,twitter_data):
        ...
        self.source, self.relevancy, self.city = self.get_source (self.link), self.get_relevancy(self.link), self.get_city (self.link)

        self.address = self.fix_abbr (self.address)

        self.all_indications, self.main_indication, self.facts_injured, self.facts_dead = self.read_tweet(self.tweet_text)
    ...
    def indication_finder(self,string):
        ...
        if (("Shot person (injured)"in indications) or ("Shot person (dead)"in indications)) and "Shots fired" in indications:
            indications.remove("Shots fired")
        if (("Shot person (injured)"in indications) or ("Shot person (dead)"in indications)) and "Sound of gunshots" in indications:
    ...
    def finalize_description(self,data):
        ...
        else: description2 = f"{description} in the area of {address} ."
