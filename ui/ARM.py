...
from json_data import file_manager
...
class ARM_Main(QtWidgets.QMainWindow):
    ...
    def _set_tooltips(self):
        dictionary_tooltips = {self.clear_indications: "Clears selected indications",
                               self.sound_of_gunshots: "Sound of gunshots", self.police_ats: "Police at the scene",
                               self.dead: "Insert the number of dead persons here",
                               self.firefighters_ats: "Firefighters at the scene", self.shots_fired: "Shots fired",
                               self.stabbing: "Stabbed person", self.police_investigation: "Police investigation",
                               self.ambulance_ats: "Ambulance at the scene",
                               self.injured: "Insert the number of injured persons here", self.shooting: "Shot person",
                               self.indications_list: "List of report indications",
                               self.add_indication: "Add an indication",
                               ...
                               self.address_type: "Type of address recognized by the reader",
                               self.coordinates: "Geographical coordinates obtained from Google Maps",
                               self.city_state: "City/State", self.location_accuracy: "Location accuracy in meters",
                               self.trust_score: "", self.address: "Street name",
                               self.duration: "Duration of the incident, remains on AVA\'s default duration if left empty",
                               self.enter_tweet_link: "Run webcrawler for the current tweet link",
                               self.tweet_text: "Original tweet text",
                               self.tweet_link_clear: "Reset all of the text fields", self.tweet_link: "Tweet link",
                               self.description: "Generated report description",
                               self.indication_selection: "Search for indications in the tree",
                               self.add_twitter_accounts: "Add a source to the table",
                               self.remove_twitter_accounts: "Removes the selected source from the table",
                               self.add_template: "Add a custom text template to the table",
                               self.remove_template: "Removes the selected template from the table"}
        for keys, values in dictionary_tooltips.items():
            keys.setToolTip(values)

    def _set_text(self):
        dictionary_set_text = {self.clear_indications: "Clear", self.dead_label: "Dead", self.injured_label: "Injured",
                               self.add_indication: "Add", self.run_bot: "Run Bot",
                               self.label_trust_score: "Trust score:", self.label_location_accuracy: "Accuracy:",
                               self.label_address: "Address:", self.label_time_accuracy: "Accuracy (+/-)",
                               ...
                               self.twitter_accounts_label: "Twitter Accounts:", self.templates_label: "Templates:",
                               self.remove_regex_address_formula: "-", self.add_regex_address_formula: "+",
                               self.edit_regex_address_formula: "Edit",
                               self.chrome_headless: "Run Chrome in headless mode",
                               self.load_images: "Load images in pages",
                               self.chrome_user_profile_label: "Chrome User Profile:",
                               self.default_time_zone_label: "Default time zone (GMT):",
                               self.ava_username_label: "AVA Username:", self.ava_username_button: "...",
                               self.chrome_user_profile_button: "...", self.add_twitter_accounts: "+",
                               self.add_template: "+", self.remove_twitter_accounts: "-", self.remove_template: "-",
                               self.edit_template: "Edit"}
        for keys, values in dictionary_set_text.items():
            keys.setText(values)

    def _set_placeholders(self):
        dictionary_set_placeholder = {self.tweet_link: "Tweet link", self.tweet_text: "Tweet text",
                                      self.description: "Report description", self.account: "Twitter acount name",
                                      self.coordinates: "Coordinates", self.house_number: "House nr.",
                                      self.address: "Street name", self.city_state: "City/State",
    ...
    def _set_sources_tab(self):
        self.remove_regex_indication_formula.clicked.connect(
            lambda: self._remove_table_item_dialog(self.regex_indication_formulas_table))
        self.remove_regex_address_formula.clicked.connect(
            lambda: self._remove_table_item_dialog(self.regex_address_formulas_table))
        self.remove_twitter_accounts.clicked.connect(
            lambda: self._remove_table_item_dialog(self.twitter_accounts_table))
    ...
    def _set_ava_profile(self):
        ...
        if ok:
            file_manager.edit_setting('ava_username', response)
        self._get_settings()
        self.ava_username.setPlainText(self.ava_username_value)
    ...
    def _set_directory(self):
        ...
        response = QFileDialog.getExistingDirectory(self, caption="Choose the Google Chrome profile folder: ",
                                                    directory=user_path)
        if response:
            file_manager.edit_setting('user_profile_path', response)
        self._get_settings()
        self.chrome_user_profile.setPlainText(self.chrome_profile_dir_setting)

    def _set_checkbox(self, checkbox, setting):
        if checkbox.checkState() == 2:
            file_manager.edit_setting(setting, True)
        else:
            file_manager.edit_setting(setting, False)
    ...
    def _add_item_to_table(self, dict_name, item):
        old_dict = file_manager.load_sources(dict_name)
        ...
        file_manager.edit_sources(dict_name, new_dict)
        self._get_dictionaries()
        self._fill_source_tables()
    ...
    def _build_indication_tree(self):
        ...
        self.tree_data = file_manager.load_indications_json()
    ...
    def toggle_indications(self):
        self.buttons_dict = {"Shots fired": self.shots_fired, "Sound of gunshots": self.sound_of_gunshots,
                             "Police investigation": self.police_investigation, "Shot person (injured)": self.shooting,
                             "Shot person (dead)": self.shooting, "Stabbed person (injured)": self.stabbing,
                             "Stabbed person (dead)": self.stabbing, "Police at the scene": self.police_ats,
                             "Ambulance at the scene": self.ambulance_ats,
                             "Firefighters at the scene": self.firefighters_ats,
                             "Emergency responders at the scene": self.emergency_services}
    ...
    def load_data_from_dict(self):
        self.tweet_data = file_manager.load_report_data()
        self.clear_fields()
    ...
    def _get_dictionaries(self):
        ...
        self.dictionary_descriptions, self.dictionary_sources, self.dictionary_additional_facts = file_manager.load_sources()
    ...
    def _get_css(self):
        self.setStyleSheet(file_manager.load_css())

    def clear_fields(self):
        fields_list = [self.tweet_link, self.tweet_text, self.description, self.indications_list, self.account,
                       self.coordinates, self.house_number, self.address, self.city_state, self.time_accuracy,
                       self.duration]
    ...
    def get_data(self):
        ...
        self.tweet_data['all_indications'] = [self.indications_list.item(x).text() for x in
                                              range(self.indications_list.count())]
        ...
        self.tweet_data['starting_time'][0], self.tweet_data['starting_time'][1] = self.time.time().toString(
            "HH,mm").split(',')
        ...
        file_manager.write_report_json(self.tweet_data)
